...
// Validasi otorisasi: Pastikan hanya guru yang sudah login yang bisa mengakses halaman ini
if (!isset($_SESSION['guru_id']) || empty($_SESSION['guru_id'])) {
    header("Location: ../login.php");
    exit;
}

// Ambil ID guru dari sesi dan informasi dasar
...
// Ambil id_jadwal dari URL dan sanitasi
...
// --- Handle Form Submission (Tambah & Edit Pertemuan) ---
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($tanggal) || empty($topik) || empty($id_jadwal_form)) {
        $error = "Tanggal dan topik pertemuan wajib diisi!";
    } else {
        try {
            if ($id_pertemuan) {
                // Update pertemuan yang sudah ada
                $stmt = $pdo->prepare("UPDATE pertemuan SET tanggal = ?, topik = ? WHERE id = ? AND id_jadwal = ?");
            ...
            // Redirect dengan pesan sukses untuk menghindari pengiriman ulang form
            header("Location: pertemuan_guru.php?id_jadwal=" . urlencode($id_jadwal_form) . "&success=" . urlencode($success));
            exit;
...
// --- Ambil pesan sukses/error dari URL (setelah redirect) ---
...
// --- Ambil data jadwal untuk konteks judul halaman ---
$stmt_jadwal_info = $pdo->prepare("
    SELECT
        j.hari, j.jam_mulai, j.jam_selesai,
        g.name AS nama_guru, m.nama_mapel, c.nama_kelas, g.photo AS guru_photo
    FROM jadwal AS j
...
// Jika jadwal tidak ditemukan atau tidak terkait dengan guru ini, redirect
if (!$jadwal_info) {
    header("Location: jadwal_guru.php?error=" . urlencode("Jadwal tidak ditemukan atau Anda tidak memiliki akses."));
    exit;
}

// Perbarui foto guru dari data jadwal yang lebih valid
$guru_photo = htmlspecialchars($jadwal_info['guru_photo']);


// --- Ambil semua data pertemuan untuk jadwal ini ---
$stmt_pertemuan = $pdo->prepare("SELECT id, tanggal, topik FROM pertemuan WHERE id_jadwal = ? ORDER BY tanggal DESC");
